"""Centralized preprocessing for the NATICUSdroid dataset.

This module loads the raw CSV, validates the binary classification target,
creates a reproducible stratified 70/30 split, and optionally persists the
processed artifacts so all notebooks can reuse the same split.
"""
...
def resolve_data_path(preferred_path: Optional[str | Path] = None) -> Path:
    """Return the first valid CSV path for the dataset."""
...
def load_dataset(preferred_path: Optional[str | Path] = None) -> pd.DataFrame:
    """Load the dataset and normalize basic schema details."""
...
def validate_dataset(dataframe: pd.DataFrame, target_column: str = DEFAULT_TARGET_COLUMN) -> None:
    """Validate the expected binary classification structure."""
...
def summarize_dataset(dataframe: pd.DataFrame, target_column: str = DEFAULT_TARGET_COLUMN) -> Dict[str, Any]:
    """Return a compact summary for notebooks and reports."""
...
def split_dataset(
    ...
    """Create the reproducible stratified split used by all notebooks."""
...
def build_artifact_paths(output_dir: Path) -> DatasetSplitArtifacts:
    """Build the standard output paths inside the processed data folder."""
...
def save_split_artifacts(
    ...
    """Persist the train/test split as CSV files plus a metadata JSON."""
...
def prepare_dataset(
    ...
    """Load, validate, split, and persist the dataset artifacts."""
